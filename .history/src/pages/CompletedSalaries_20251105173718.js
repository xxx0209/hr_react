...
export default function CompletedSalaries() {
  const [list, setList] = useState([]); // 전체 승인 급여
  const [filteredList, setFilteredList] = useState([]); // 검색 필터링된 목록
  ...
  const fetchData = async () => {
    try {
      ...
      setFilteredList(salariesRes.data); // 초기에는 전체 목록 보여주기
    } catch (err) {
  ...
  const handleFilterChange = (e) => {
    ...
    const filtered = list.filter((s) => {
      const matchMember = newFilters.memberId
        ? s.memberId === Number(newFilters.memberId)
        : true;

      const matchMonth = newFilters.salaryMonth
        ? s.salaryMonth?.startsWith(newFilters.salaryMonth) // 안전하게 처리
        : true;
