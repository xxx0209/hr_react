...
const BaseSalaryPage = () => {
  ...
  const handleSave = async () => {
    ...
    const existing = baseSalaries.find(s =>
      type === 'POSITION' ? s.position.positionName === referenceId : s.member.id === referenceId
    );
  ...
  const isExisting = baseSalaries.some(s =>
    type === 'POSITION' ? s.position.positionName === form.referenceId : s.member.id === form.referenceId
  );
