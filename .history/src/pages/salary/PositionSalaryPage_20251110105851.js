...
function PositionSalaryPage() {
  ...
  const [salaries, setSalaries] = useState([]);   // 직급별 급여 목록
  ...
  const fetchData = async () => {
    try {
      const [posRes, salaryRes] = await Promise.all([
        ...
        axios.get("/api/position-salaries?page=0&size=100") // 페이징 적용된 전체 목록
      ]);
      setPositions(posRes.data); 
      setSalaries(salaryRes.data.content || []); // Page 객체의 content 사용
    } catch (err) {
      console.error("데이터 로딩 실패", err);
      alert("직급 또는 급여 데이터를 가져오는 중 오류가 발생했습니다.");
    }
  };
