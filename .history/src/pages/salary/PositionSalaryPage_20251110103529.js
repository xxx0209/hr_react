...
function PositionSalaryPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      setTotalPages(salaryRes.data.totalPages || 0);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await axios.put(`/api/position-salaries/${formData.id}`, formData);
      } else {
  ...
  return (
    <div className="container mt-4">
      ...
      <Row className="mb-3">
        <Col md={6}>
          <Button onClick={() => handleShow()}>+ 급여 등록</Button>
        </Col>
        <Col md={6}>
          <Form.Control
            type="text"
            placeholder="검색 (직급/제목)"
            value={search}
        ...
        <Modal.Body>
          <Form>
            <Form.Group className="mb-3">
              ...
              <SelectCombo
                options={positions.map((p) => ({ label: p.positionName, value: p.id }))}
