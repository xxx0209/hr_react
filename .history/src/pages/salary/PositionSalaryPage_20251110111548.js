...
function PositionSalaryPage() {
  ...
  const [searchPositionId, setSearchPositionId] = useState(""); // 검색용

  useEffect(() => {
    fetchData();
  }, [page, size, searchPositionId]);
    ...
    try {
      const [posRes, salaryRes] = await Promise.all([
        ...
        axios.get("/api/position-salaries", {
          params: { page, size, search: searchPositionId },
        }),
      ]);
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="d-flex mb-3">
        <SelectCombo
          options={[{ label: "전체", value: "" }, ...positions.map((p) => ({ label: p.positionName, value: p.positionId }))]}
          value={searchPositionId}
          onChange={(v) => {
            setSearchPositionId(v);
