  ...
  const fetchData = async () => {
    try {
      const [posRes, salaryRes] = await Promise.all([
        axios.get('/position/all'), // Page 반환
        axios.get('/api/position-salaries')
      ]);

      // Page.content에서 직급 데이터 추출
      setPositions(posRes.data.content.map(p => ({
        value: p.positionId,
          ...
          {salaries.length === 0 ? (
            ...
            salaries.map((s) => (
              <tr key={s.id}>
                ...
                <td>
                  <Button variant="warning" size="sm" className="me-2" onClick={() => handleShow(s)}>수정</Button>
