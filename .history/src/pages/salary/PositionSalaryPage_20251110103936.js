...
function PositionSalaryPage() {
  ...
  const fetchData = async () => {
    try {
      const posRes = await axios.get('/position/all');
      const salaryRes = await axios.get('/api/position-salaries');

      // Page 객체 대응
      setPositions(posRes.data.content || posRes.data); // content 있으면 Page, 없으면 배열
      setSalaries(salaryRes.data.content || salaryRes.data);
    } catch (err) {
      console.error(err);
    }
  };
    ...
    <div className="container mt-4">
          ...
          {salaries.length === 0 ? (
            ...
            salaries.map((s) => (
              <tr key={s.id}>
                ...
                <td>
                  <Button variant="warning" size="sm" onClick={() => handleShow(s)} className="me-2">수정</Button>
      ...
      <Modal show={show} onHide={() => setShow(false)}>
          ...
          <Form>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                onChange={(e) => {
                  ...
                  if (!isNaN(rawValue)) setFormData({ ...formData, baseSalary: rawValue });
                }}
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                onChange={(e) => {
                  const rawValue = e.target.value.replace(/,/g, '');
                  if (!isNaN(rawValue)) setFormData({ ...formData, hourlyRate: rawValue });
                }}
                placeholder="시급 입력 (원)"
              />
            </Form.Group>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleSave}>{editing ? "수정 저장" : "등록"}</Button>
        </Modal.Footer>
      </Modal>
    </div>
  );
