...
function MemberSalaryPage() {
  ...
  const fetchMembers = async () => {
    try {
      ...
      setMembers(res.data); // map 하지 않고 그대로 넣기
    } catch (error) {
    ...
    <div className="container mt-4">
      ...
      <div className="d-flex mb-3">
        <SelectCombo
          ...
          onChange={(v) => {
            // 검색용으로 label을 백엔드 search로 전달
            const selectedMember = members.find(m => m.value === v);
            setSearchMemberId(selectedMember ? selectedMember.label : "");
            setPage(0);
          }}
          placeholder="회원 선택"
          searchable
        ...
        <Modal.Body>
          <Form>
            <Form.Group className="mb-3">
              ...
              <SelectCombo
                options={members} // {label, value}로 변환됨
