          ...
          {list.length === 0 ? (
            ...
            list.map((s) => (
              <tr key={s.salaryId}>
                ...
                <td>{s.status}</td>
                <td>
                  <Button size="sm" variant="warning" onClick={() => handleEdit(s)} className="me-1" disabled={s.status === "COMPLETED"}>수정</Button>
                  <Button size="sm" variant="danger" onClick={() => handleDelete(s.salaryId, s.status)} className="me-1" disabled={s.status === "COMPLETED"}>삭제</Button>
