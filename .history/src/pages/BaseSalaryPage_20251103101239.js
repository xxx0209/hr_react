...
const BaseSalaryPage = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        setPositions(posRes.data); // 원본 유지
  ...
  const positionComboOptions = positions.map(p => ({
    value: p.positionCode, // 기준급 등록 시 사용할 코드
    label: p.name          // 드롭다운에 표시될 이름
  }));

  const memberComboOptions = members.map(m => ({
    value: m.id,
    label: m.name
  }));
  ...
  const handleAddSalary = async (type) => {
    ...
    const payload = {
      type,
      referenceId: target.referenceId, // positionCode 또는 memberId
