    ...
    if (memberSalary) { 
      setForm({
         ...form,
          ...
          try { const res = await axios.get(/api/position - salaries / member / ${ memberId }); setForm({ ...form, memberId, salaryType: "POSITION", baseSalary: "", hourlyRate: "", positionSalaryId: "", availablePositionSalaries: res.data, }); } catch (err) { console.error("직급 급여 불러오기 실패", err); } }
