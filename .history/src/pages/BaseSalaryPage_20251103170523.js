...
const BaseSalaryPage = () => {
  ...
  useEffect(() => {
    const fetchReferenceOptions = async () => {
      try {
        ...
        const options = res.data.map(item => ({
          value: type === 'POSITION' ? item.positionId : item.id,
    ...
    const fetchBaseSalaries = async () => {
      try {
        const res = await axios.get(`/api/base-salaries?type=${type}`);
  ...
  const isExisting = baseSalaries.some(s => s.referenceId === form.referenceId);
      ...
      <table border="1" cellPadding="8" style={{ width: '100%', backgroundColor: '#fff' }}>
        ...
        <tbody>
          {baseSalaries.map((s, idx) => {
            const label = referenceOptions.find(r => r.value === s.referenceId)?.label || s.referenceId;
            return (
              <tr key={s.id || idx}>
                <td>{label}</td>
