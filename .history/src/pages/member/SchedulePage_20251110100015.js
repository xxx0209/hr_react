...
import TwoWeekView from "./TwoWeekView"; // 위 코드 경로에 맞게 import
...
export default function SchedulePage() {
    ...
    // 1️⃣ 2주짜리 커스텀 뷰 생성
    const TwoWeekView = (props) => {
        const { date, localizer } = props;

        // 오늘을 기준으로 2주 날짜 범위 계산
        const start = startOfWeek(new Date(), { weekStartsOn: 0 }); // 일요일 시작
        const end = addDays(start, 13); // 2주 (14일)
        ...
        return (
            <Views.Week
                ...
                date={date}
                localizer={localizer}
                range={[...Array(14)].map((_, i) => addDays(start, i))}
            />
        );
    };
