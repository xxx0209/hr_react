...
export default function SalaryManager() {
  ...
  // 🔥 필터링된 직급 급여 목록 상태 추가
  const [filteredPositionSalaries, setFilteredPositionSalaries] = useState([]);
  ...
  const fetchData = async () => {
    const [membersRes, memberSalaryRes, positionsRes, salariesRes] = await Promise.all([
  ...
  const handleClose = () => {
    ...
    setFilteredPositionSalaries([]); // 초기화
  };

  // 🔥 회원 선택 시 직급별 급여 필터링 추가
  const handleMemberChange = (e) => {
    ...
    if (memberSalary) {
      ...
      setFilteredPositionSalaries([]); // 직급 급여 비활성
    } else {
      // ✅ 개인 급여 없음 → 해당 직급의 급여만 표시
      const filtered = positionSalaries.filter(
        (ps) => ps.position?.positionId === member.position?.positionId
      );
      setFilteredPositionSalaries(filtered);
  ...
  const handlePositionSalaryChange = (e) => {
    ...
    const positionSalary = positionSalaries.find((ps) => ps.id === id);
            ...
            <Row className="mb-3">
              <Col md={6}>
                <Form.Group>
                  ...
                  <Form.Select value={form.memberId} onChange={handleMemberChange} required>
              ...
              <Col md={6}>
                <Form.Group>
                  ...
                  >
                    <option value="">선택</option>
                    {filteredPositionSalaries.map((p) => (
