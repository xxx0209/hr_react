...
const SalaryForm = () => {
  ...
  const [positionList, setPositionList] = useState([]); // 직급 이름 포함
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setPositionOptions(posSalRes.data);
        ...
        const positionComboOptions = positionOptions.map(p => {
          const matched = positionList.find(pos => pos.positionCode === p.referenceId);
          return {
            value: p.referenceId,
            label: matched ? matched.name : p.referenceId
          };
        });
      } catch (err) {
