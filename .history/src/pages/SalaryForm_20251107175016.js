...
export default function SalaryManager() {
  ...
  const handleMemberChange = async (memberId) => {
    ...
    const memberSalary = memberSalaries.find((ms) => ms.memberId === memberId); if (memberSalary) { setForm({ ...form, memberId, salaryType: "MEMBER", baseSalary: memberSalary.baseSalary, hourlyRate: memberSalary.hourlyRate, positionSalaryId: "", availablePositionSalaries: [], }); } else { try { const res = await axios.get(/api/position - salaries / member / ${ memberId }); setForm({ ...form, memberId, salaryType: "POSITION", baseSalary: "", hourlyRate: "", positionSalaryId: "", availablePositionSalaries: res.data, }); } catch (err) { console.error("직급 급여 불러오기 실패", err); } }
