...
const BaseSalaryPage = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        const sortedPositions = posRes.data.sort((a, b) => a.positionName.localeCompare(b.positionName));
  ...
  const isExisting = positionSalaries.some(s => s.positionId === newPositionSalary.referenceId);
    ...
    <div style={{ maxWidth: '900px', margin: '40px auto', fontFamily: 'Segoe UI, sans-serif' }}>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '10px', flexWrap: 'wrap' }}>
          ...
          />
          {isExisting ? (
            <button style={buttonStyle} onClick={handleUpdateSalary}>수정</button>
          ) : (
            <button style={buttonStyle} onClick={handleAddSalary}>등록</button>
          )}
        </div>
            ...
            {positionSalaries.map((s, idx) => {
              ...
              return (
                <tr key={s.id || idx}>
                  ...
                  <td>
                    <button style={{ ...buttonStyle, backgroundColor: '#d32f2f' }} onClick={() => handleDeleteSalary(s.id)}>삭제</button>
                  </td>
                </tr>
              );
            })}
